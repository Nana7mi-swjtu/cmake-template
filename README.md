...
同步在**每次运行 ctpl 时**自动做（不需要重启，也没有缓存）：判断依据是模板内容的哈希，所以工具里的内置模板一改（哪怕 `version` 没动），你本地的副本下次运行就会跟上，并打印 `✔ 已更新内置模板 …`。
若你改过本地副本，工具不会动它；想换成官方版本：`ctpl restore default-cpp --yes`（先备份你的版本）。

> 注意：`ctpl init` 反向生成的**你自己的**模板不属于内置模板，工具不会碰它。内置模板修了 bug 后，这类模板需要重新 `ctpl init`（或照着新版模板改一下 `files/` 里的文件）。
